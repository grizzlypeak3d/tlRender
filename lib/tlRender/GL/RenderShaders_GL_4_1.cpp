    ...
    namespace gl
    {
            ...
            LUTOrder lutOrder)
        {
            ...
            return ftk::Format(
                ...
                "}\n"
                "\n"
                "// The weight along one axis: the kernel where that axis is\n"
                "// enlarged, and otherwise the nearest texel alone, which at\n"
                "// the one to one size the picture is then drawn at is the\n"
                "// texel itself.\n"
                ...
                "            vec4 texel = texture(s, clamp(t, vec2(0.0), vec2(1.0)));\n"
                "            c += w * texel;\n"
                "            total += w;\n"
                "            if (i >= 0 && i <= 1 && j >= 0 && j <= 1)\n"
                "            {\n"
                ...
                "    // Held to the four texels around the sample. The kernel's\n"
                "    // negative lobes overshoot a hard edge -- a ring below\n"
                "    // black and above white that the picture does not have,\n"
                "    // and that the clipping warning then reports.\n"
        ...
        std::string clippingWarningFragmentSource()
        {
            return
                ...
                "{\n"
                "    vec4 c = texture(textureSampler, fTexture);\n"
                // What is compared with the top and the bottom of the range:
                // any channel is the largest above and the smallest below,
                // all channels the other way around, and the luminance both.
