...
var BLOCK = 1024 * 1024;
// The compressed cache carries the depth that the decoded cache
// cannot afford: these are worker-side buffers the garbage collector
// can reclaim, while every decoded frame lands in the wasm heap,
// which never shrinks.
var CACHE_BLOCKS = 128;

// Safari's collector falls behind sustained allocation in a busy
// worker -- the process grows until the page is killed -- so the
// short-lived buffers are pooled per source instead of left as
// garbage. Buffers are handed out as views over power-of-two backing
// stores; only backings the pool made are ever taken back.
function poolOf(S)
{
    if (!S.pool)
    {
        ...
        S.pool = {
            ...
            {
                return n > 0 ?
                    new Uint8Array(backing(n), 0, n) :
                    new Uint8Array(0);
            },
            floats: function(n)
            {
                return n > 0 ?
                    new Float32Array(backing(n * 4), 0, n) :
                    new Float32Array(0);
            },
            drop: function(view)
            {
                if (view && view.buffer && view.buffer.__pool &&
                    0 === view.byteOffset &&
                    held + view.buffer.byteLength <= HOLD_MAX)
                {
                    var c = classOf(view.buffer.byteLength);
                    (free[c] = free[c] || []).push(view.buffer);
                    held += view.buffer.byteLength;
                }
            }
        };
    }
...
// A view of the byte range: a subarray of the whole-file buffer where
// there is one (no copy at all), a pooled assembly from the block
// cache where there is not. The codec chunks copy at construction, so
// a view is enough -- the caller drops it back to the pool when the
// chunk is made.
function readRangeView(S, offset, size)
{
    ...
    if (S.buffer)
    {
        return Promise.resolve(
            new Uint8Array(S.buffer, offset, end - offset));
    }
    if (S.head && end <= S.head.byteLength)
    {
        return Promise.resolve(
            new Uint8Array(S.head, offset, end - offset));
    }
    ...
    {
        var out = poolOf(S).bytes(end - offset);
...
// A copy of the byte range, for a caller that keeps it.
function readRange(S, offset, size)
{
    return readRangeView(S, offset, size).then(function(v)
    {
        var ab = new ArrayBuffer(v.length);
...
function ensureAudio(A, start, count)
{
    ...
    for (var i = d0; i <= f1; ++i)
    {
        ...
        {
            reads.push(
                readRangeView(A.src, A.frames[i].off, A.frames[i].size)
    ...
    return Promise.all(reads).then(function(datas)
    {
        return new Promise(function(resolve, reject)
        {
            ...
            // The chunks copied the data at construction.
            for (var i = 0; i < datas.length; ++i)
            {
                poolOf(A.src).drop(datas[i]);
            }
            A.adec.flush().then(function()
            {
                ...
                for (var k in A.cache)
                {
                    ...
                    if (pos < lo || pos > hi)
                    {
                        var planes = A.cache[k].planes;
                        for (var c = 0; c < planes.length; ++c)
                        {
                            poolOf(A.src).drop(planes[c]);
                        }
                        delete A.cache[k];
                    }
                }
...
function open(R)
{
    ...
    R.prefetch = function()
    {
        ...
        for (var i = R.next; i <= last; ++i)
        {
            ...
            (function(i)
            {
                R.pending[i] = true;
                var s = R.samples[i];
                readRangeView(R.src, s.off, s.size).then(function(v)
                {
                    delete R.pending[i];
                    R.store[i] = v;
                    R.pump();
    ...
    R.pump = function()
    {
            ...
            R.queue.length < 8)
        {
            ...
            // The chunk copied the data at construction.
            poolOf(R.src).drop(d);
        }
        R.prefetch();
        if (R.dec &&
