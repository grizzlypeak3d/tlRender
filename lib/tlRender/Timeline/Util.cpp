...
namespace tl
{
        ...
        const ftk::DirListOptions& options)
    {
        ...
        if (std::filesystem::is_directory(ftk::toFileSystem(path.get())))
        {
            ...
            // toFileSystem for the reason the line above uses it: the name
            // is UTF-8, and the implicit conversion would read it as the
            // Windows ANSI code page. See Timeline.cpp and issue #779.
